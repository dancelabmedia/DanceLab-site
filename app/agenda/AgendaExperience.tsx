    ...
    <article className="srt-card" style={{ animationDelay: `${animDelay}ms` }}>
      ...
      <div className="srt-card-media">
        {event.image ? (
          ...
          >
            <span className="srt-placeholder-cat" style={{ color }}>
              {event.category}
            </span>
          </div>
        )}
        ...
        <div className="srt-card-meta">
          ...
          <span className={isFree(event) ? "srt-meta-free" : ""}>
            <svg width="11" height="11" viewBox="0 0 14 14" fill="none" aria-hidden="true">
              ...
              <path
                d="M7 4v6M5 5.5h2.5a1.5 1.5 0 0 1 0 3H5"
                stroke="currentColor"
                strokeWidth="1.3"
                strokeLinecap="round"
              />
...
export default function AgendaExperience({ events }: AgendaExperienceProps) {
  ...
  const dateOptions  = [ALL_DATES, "En cours", "À venir"]
  const priceOptions = [ALL_PRICES, "Gratuit", "Payant", "À compléter"]
  const sortOptions  = [SORT_RECENT, SORT_OLD]
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFilter  === "En cours") r = r.filter((e) => e.status === "En cours")
    if (dateFilter  === "À venir")  r = r.filter((e) => e.status !== "En cours")
    if (priceFilter === "Gratuit")  r = r.filter((e) => isFree(e))
    if (priceFilter === "Payant")   r = r.filter((e) => !isFree(e) && e.price !== "À compléter")
    if (priceFilter === "À compléter") r = r.filter((e) => e.price === "À compléter")
  ...
  return (
    <>
        ...
        <div className="container">
          ...
          <div className="srt-filter-row">
            <div className="srt-filters">
              ...
              <Dropdown label="Date"  value={dateFilter}  options={dateOptions}  onChange={setDateFilter}  />
              <Dropdown label="Prix"  value={priceFilter} options={priceOptions} onChange={setPriceFilter} />
              {hasActiveFilter && (
      ...
      {/* ════ Map — AgendaMap gère sa propre section ════ */}
      <AgendaMap
        events={filtered}
